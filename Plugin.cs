...
{
    [BepInPlugin("LCMamehinata", "LethalMamehinata", "0.1.0")]
    ...
    public class Plugin : BaseUnityPlugin
    {
        ...
        {
                ModelReplacementAPI.RegisterModelReplacementOverride(typeof(MRMAMEHINATA));

            }
            if (enableModelAsDefault.Value)
            {
                ModelReplacementAPI.RegisterModelReplacementDefault(typeof(MRMAMEHINATA));

            }
            var commaSepList = suitNamesToEnableModel.Value.Split(',');
            foreach (var item in commaSepList)
            {
                ModelReplacementAPI.RegisterSuitModelReplacement(item, typeof(MRMAMEHINATA));
            }
                

            Harmony harmony = new Harmony("LCMamehinata");
            harmony.PatchAll();
            Logger.LogInfo($"Plugin {"LCMamehinata"} is loaded!");
        }
    }
